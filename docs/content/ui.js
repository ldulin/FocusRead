...
(function (root) {
  ...
  var STYLE = [
    ...
    /* Narrow screens - a phone, or a split window. The bar has ten controls
       and would otherwise overflow the viewport with no way to reach the ones
       past the edge. Let it wrap, and grow the touch targets: 32px squares are
       under the ~44px a finger needs. */
    '@media (max-width:560px){',
    /* An explicit width matters: a fixed, wrapping flex container otherwise
       shrink-wraps to its narrowest fit and stacks into a tall square block
       sitting on top of the text. */
    '  .bar{flex-wrap:wrap;justify-content:center;width:96vw;max-width:96vw;',
    '       row-gap:2px;column-gap:1px;bottom:10px;padding:5px 4px}',
    '  .grip{display:none}',                       /* dragging is awkward by touch */
    '  button{min-width:42px;height:42px}',
    '  button.primary{min-width:52px}',
    '  select[data-role="voice"]{display:none}',
    '  .count{font-size:13px;padding:0 3px}',
    '  .sep{display:none}',
    '  .rate{min-width:44px}',
    '  .pop{max-width:94vw}',
    '  .toast{bottom:120px;max-width:90vw}',
    '}',

    /* Pointer: coarse catches tablets and phones regardless of width. */
  ...
  /** Phone-ish: too narrow for a ten-control bar, or driven by touch. */
  function narrowScreen() {
    try {
      return root.innerWidth < 640 ||
  ...
  UI.prototype._makeDraggable = function (bar, grip) {
    ...
    grip.addEventListener('pointerup', function (e) {
      ...
      if (self.actions.moved && !narrowScreen()) {
        self.actions.moved({ left: parseFloat(bar.style.left), top: parseFloat(bar.style.top) });
      }
    });

    // Restore a previously dragged position - but not on a phone. A position
    // chosen on a desktop leaves the bar stranded somewhere in the middle of a
    // narrow screen, on top of the text, and there is no grip to drag it back
    // by. Narrow screens always get the default centred position.
    var saved = narrowScreen() ? null : this.opts.position;

    // The clamp has to wait until the host is in the document: measuring
    // offsetWidth on a detached element returns 0, so every restored position
